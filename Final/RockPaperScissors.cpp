...
int main()
{
    ...
    //시작 메뉴 창
    Mat menu(menu_h, menu_w, CV_8UC3, Scalar(255, 255, 255));

    // menu에 이미지를 삽입할 위치와 크기를 지정합니다.
    int x = 400;
    int y = 150;
    int width = 519;
    int height = 168;
    ...
    // 삽입할 이미지를 로드
    Mat image = imread("../image/rps2.png", IMREAD_COLOR);

    // 이미지가 로드되었는지 확인
    if (!image.empty()) {
        // 이미지가 menu 영역에 들어갈 수 있는지 확인
        if (x >= 0 && y >= 0 && x + width <= menu.cols && y + height <= menu.rows) {
            // menu의 지정된 영역에 이미지를 복사
            Mat roi = menu(Rect(x, y, width, height));
            image.copyTo(roi);
        }
    }

    // 자주 사용할 컬러 이미지를 저장할 Mat 개체 선언
    Scalar red(0, 0, 255);
    Scalar blue(255, 0, 0);
    ...
    //시작 메뉴에 텍스트 추가
    int thickness = 2;
    Point location(center_x - 500, center_y - 100); //글씨 위치
    int font = FONT_HERSHEY_TRIPLEX;// 폰트
    double fontScale = 3; //글씨 크기
    putText(menu, "Rock Scissors Paper", location, font, fontScale, yellow, 3); //텍스트 삽입

    location = Point(center_x - 250, center_y + 40);
    font = FONT_ITALIC;
    fontScale = 1;
    putText(menu, "Press ANY KEY to start game", location, font, fontScale, red, thickness);

    location = Point(center_x - 350, center_y + 180);
    font = FONT_HERSHEY_SIMPLEX;
    fontScale = 1;
    putText(menu, "place your palm in blue circle for 5 seconds", location, font, fontScale, blue, thickness);
    ...
    while (1) {
        ...
        cam.set(CAP_PROP_FRAME_HEIGHT, 400);
        Mat Origin_frame;
        ...
        while (cam.read(Origin_frame)) {
            ...
            resize(HandMask, HandMask, Size(Origin_frame.cols, Origin_frame.rows), 0, 0, INTER_LINEAR);
            ...
            cvtColor(origin_mask, YCrCb, COLOR_BGR2YCrCb);  //origin_mask(원본 이미지에 마스크를 적용)이미지를 YCrCb 컬러 모델로 변환

            /*피부 영역 추출*/
            Mat skin_area;
            inRange(YCrCb, Scalar(0, 130, 90), Scalar(255, 168, 130), skin_area);   //정해진 범위에서 픽셀 값을 각각 0과 255로 변환해 피부 영역을 추출한다

            /*피부 영역 Gray Scale 이미지에 적용*/
            Mat Mask_gray; //마스크를 저장한 카메라 이미지의 gray scale 이미지를 저장할 변수
            cvtColor(origin_mask, Mask_gray, COLOR_BGR2GRAY); //마스크를 저장한 카메라 이미지를 gray scale로 변환
            Mat Skin_gray = Mask_gray + (~skin_area);         //gray scale 이미지에 피부 영역의 반전 영상을 더해 피부 영역만 gray sclae로 추출(~연산은 이미지를 반전시킨다)
            ...
            /*거리 변환 함수 적용*/
            Mat dist;
            distanceTransform(tild_thresh_skin_gray, dist, DIST_L2, 5);
        ...
        //가위 바위 보 결과 창
        Mat resultWindow(menu_h, menu_w, CV_8UC3, Scalar(255, 255, 255));

        //결과 창 텍스트 삽입
        Point location1(center_x - 160, center_y - 200);
        font = FONT_HERSHEY_TRIPLEX;
        putText(resultWindow, "Result", location1, font, 3, yellow, thickness);
        fontScale = 1;

        location1 = Point(center_x - 430, center_y - 80);
        putText(resultWindow, "PLAYER", location1, FONT_ITALIC, 2, red, thickness);

        location1 = Point(center_x + 180, center_y - 80);
        ...
        if (result == rock) {
            printf("바위");
            // resultWindow에 이미지를 삽입할 위치와 크기를 지정
            int x = 250;
            int y = 300;
            int width = 150;
            int height = 150;
            ...
            // 삽입할 이미지를 로드
            Mat image = imread("../image/rock_user.png", IMREAD_COLOR);

            // 이미지가 로드되었는지 확인
            if (!image.empty()) {
                // 이미지가 resultWindow 영역에 들어갈 수 있는지 확인
                if (x >= 0 && y >= 0 && x + width <= resultWindow.cols && y + height <= resultWindow.rows) {
                    // resultWindow의 지정된 영역에 이미지를 복사
                    Mat roi = resultWindow(Rect(x, y, width, height));
                    image.copyTo(roi);
                }
            }
        }
        //사용자가 낸 손 모양이 가위인 경우
            ...
            // 삽입할 이미지를 로드
            Mat image = imread("../image/scissors_user.png", IMREAD_COLOR);
        ...
        //사용자가 낸 손 모양이 보인 경우
            ...
            // 삽입할 이미지를 로드
            Mat image = imread("../image/paper_user.png", IMREAD_COLOR);
        ...
        if (random == 0) {
            ...
            // 삽입할 이미지를 로드
            Mat image = imread("../image/rock_com.png", IMREAD_COLOR);
        ...
        //컴퓨터가 가위를 낸 경우
            ...
            // 삽입할 이미지를 로드
            Mat image = imread("../image/scissors_com.png", IMREAD_COLOR);
        ...
        //컴퓨터가 보를 낸 경우
            ...
            // 삽입할 이미지를 로드
            Mat image = imread("../image/paper_com.png", IMREAD_COLOR);
        ...
        location1 = Point(center_x - 80, center_y + 170);
        font = FONT_HERSHEY_SIMPLEX;  // normal size sans-serif font
        fontScale = 3;
        putText(resultWindow, str, location1, font, fontScale, blue, thickness);

        //게임 종료, 게임 다시 플레이 안내문 삽입
        fontScale = 1;
        location1 = Point(center_x - 200, center_y + 300);
        putText(resultWindow, "Press ANY KEY to Replay            Exit with [esc]", location1, FONT_ITALIC, fontScale, red, 2);
        //게임 결과 창 출력
        while (1) {
            imshow("Result", resultWindow);
            char key1 = (char)waitKey(10);
            //아무 키 입력시 다시 메인 메뉴로 돌아감
            if (key1 != -1 && key1 != 27) {
                destroyAllWindows(); //결과 창 종료
                break;
            }
            //ESC키 입력시 프로그램 종료
            else if (key1 == 27) { exit(0); }
        }
    }
    return 0;
}